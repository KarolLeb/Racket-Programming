...
(define (make-node elem heap-a heap-b)
  (let ((ra (rank heap-a))
        (rb (rank heap-b)))
    (if (< ra rb)
        (list 'hnode elem (+ ra 1) heap-b heap-a)
        (list 'hnode elem (+ rb 1) heap-a heap-b))))
...
(define (heap-merge h1 h2)
  (define (merge e hl hr h)
    (make-node e (heap-merge hr h) hl))
  (cond
    [(leaf? h1) h2]
    [(leaf? h2) h1]
    [else (let ((e1 (node-elem h1))
                (e2 (node-elem h2)))
            (if (< (elem-priority e1)
                   (elem-priority e2))
                (merge e1 (node-left h1) (node-right h1) h2)
                (merge e2 (node-left h2) (node-right h2) h1)))]))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(define (test)
  (let ((list (randlist 500000 100)))
    #;(display list)
    #;(display "\n")
    (if (sorted? list)
        (display list)
        (display (heapsort list)))))