...
(define (sorted-varlist? x)
  (and (list? x)
       (andmap var? x);;usunięto nawiasy, było: (var? x)
       (sorted? x)))
...
(define (res-clause pos neg proof)
  (displayln (list 'res-int pos neg))
  ...
  (define (easier-iter l1 l2)
    (cond ((null? l2) true)
          ...
          ((var<? (car l1) (car l2))
           (iter (cdr l1) l2))
          (else false)))
  (if (and (easier-iter (res-clause-pos c1) (res-clause-pos c2))
           (easier-iter (res-clause-neg c1) (res-clause-neg c2))
  )))

(define (shares-member l1 l2)
  (define (member-in-sorted-list? e l)
    (if (null? l)
        false
        (if (var=? e (first l))
            true
            (if (var<? e (first l))
                (member-in-sorted-list? e (cdr l))
                false))))
  (if (null? l1)
      null
      (if (member-in-sorted-list? (car l1) l2)
          (car l1)
          (shares-member (cdr  l1) l2))))

(define (clause-trivial? c)
  ;; DONE
  (not (null? (shares-member (res-clause-pos c) (res-clause-neg c)))))

(define (resolve c1 c2)
  ;; DONE
  (let ((el (shares-member (res-clause-pos c1) (res-clause-neg c2))))
    (if (null? el)
        (let ((el2 (shares-member (res-clause-pos c2) (res-clause-neg c1))))
          (if (null? el2)
              false
              (res-clause (merge-vars (res-clause-pos c1)
                                      (remove el2 (res-clause-pos c2) var=?))
                          (merge-vars (res-clause-neg c2)
                                      (remove el2 (res-clause-neg c1) var=?))
                          (proof-res el2
                                     (res-clause-proof c2)
                                     (res-clause-proof c1)))))
        (res-clause (merge-vars (res-clause-pos c2)
                                (remove el (res-clause-pos c1) var=?))
                    (merge-vars (res-clause-neg c1)
                                (remove el (res-clause-neg c2) var=?))
                    (proof-res el
                               (res-clause-proof c1)
                               (res-clause-proof c2))))))
...
(define form-trivial (list 'cnf
                           (list 'clause
                                 (list 'literal #t 'p)
                                 (list 'literal #f 'p))))

(define form-single (list 'cnf
                          (list 'clause
                                (list 'literal #t 'p))
                          (list 'clause
                                (list 'literal #f 'p))))

(define form-easy (list 'cnf
                        (list 'clause
                              (list 'literal #t 'q)
                              (list 'literal #t 'p))
                        (list 'clause
...
(define form-single-lit (list 'cnf
                              (list 'clause
                                    (list 'literal #f 'p))))
#;(define (test x)
  (form->clauses x))
...
(define (test2 x)
  (resolve-prove null (form->clauses x)))
